...
import FanSignupComponent from '../Components/FanSignup'
import React, { useState, useRef } from 'react';
import { Regex_Validator } from '../../src/helper/constants';
import { Fan_SigninAPI } from '../../src/helper/services/API/Users';
...
const FanSignup = (props) => {
  ...
  const onSignUp = async (e) => {
    ...
    if (handleValidation()) {
      ...
      try {
        const response = await Fan_SigninAPI(params);

        if (Array.isArray(response)) {
          // If response is an array, it means it contains validation errors
          setSnackbarState({
            messageInfo: {
              open: true,
